...
def X_com_moments(time_axis, Xcom, order):
    ...
    x_all_int_fun = interp1d(time_axis,
                             np.power(time_axis, order) * x_all / duration_t,
                             kind="quadratic")
    y_all_int_fun = interp1d(time_axis,
                             np.power(time_axis, order) * y_all / duration_t,
                             kind="quadratic")
    z_all_int_fun = interp1d(time_axis,
...
def compute_com_alpha(time_i, time_f, Xcom_0, Xcom_1):
    ...
    com_alpha = (4 *
                 (time_f**2 + time_f * time_i + time_i**2) * np.array(Xcom_0) -
                 6 *
                 (time_f + time_i) * np.array(Xcom_1)) / (time_f - time_i)**2
...
def compute_com_beta(time_i, time_f, Xcom_0, Xcom_1):
    ...
    com_beta = (12 * (Xcom_1) - 6 *
                (time_f + time_i) * Xcom_0) / (time_f - time_i)**2
...
def boost_waveform(unboosted_waveform, conformal_factor):
    ...
    boosted_waveform = spherical_array(gridinfo=unboosted_waveform.gridinfo,
