...
class spherical_array:
    ...
    def to_modes_array(self, grid_info=None, spin_weight=None, ell_max=8):
        ...
        waveform_modes.create_modes_array(ell_max=ell_max,
        ...
        for mode in modes_list:
            ...
            for emm_value in all_emm_values:
                ...
                # Using quad
                multipole_ell_emm = np.tensordot(self.data,
                                                 Ydarea,
                                                 axes=((0, 1), (0, 1)))

                # print(f'l{ell_value}m{emm_value}', multipole_ell_emm)

                waveform_modes.set_mode_data(ell_value, emm_value,
                                             multipole_ell_emm)
    ...
    def boost(self, conformal_factor, grid_info=None):
        ...
        boosted_waveform_data = np.transpose(self.data,
                                             (2, 0, 1)) * conformal_factor
        ...
        boosted_waveform = spherical_array(grid_info=grid_info,
                                           data=np.transpose(
                                               np.array(boosted_waveform_data),
                                               (1, 2, 0)))
    ...
    def supertranslate(self, supertransl_alpha_sp, order=1):
        ...
        for index in range(order):
            ...
            data += np.power(supertransl_alpha_sp.data,
                             index + 1) * dPsidu / np.math.factorial(
                                 index + 1)  # delta
    ...
    def load_qlm_data(self,
        ...
        with h5py.File(full_path, "r") as wfile:
            ...
            for key in modes_keys_list:
                ...
                data_item = np.array(wfile[key])[nghosts:nphi - nghosts,
                                                 nghosts:ntheta - nghosts]
                try:
        ...
        self.invariant_coordinates_data = np.transpose(np.array(data_array),
                                                       (2, 1, 0))
        ...
        sqrt_met_det = np.sqrt(
            np.linalg.det(
                np.transpose(
                    np.array([[qtt_data_array, qtp_data_array],
                              [qtp_data_array, qpp_data_array]]),
                    (2, 3, 4, 0, 1))))

        self.sqrt_met_det_data = sqrt_met_det

    def to_shear_modes_array(self,
                             grid_info=None,
                             spin_weight=None,
                             ell_max=8):
        """Decompose a given spherical array function on a sphere
        ...
        for mode in modes_list:
            ...
            for emm_value in all_emm_values:
                ...
                Ybasis_fun = np.conj(
                    Yslm_vec(spin_weight=spin_weight,
                             ell=ell_value,
                             emm=emm_value,
                             theta_grid=theta,
                             phi_grid=phi))
                ...
                waveform_modes.set_mode_data(ell_value,
                                             emm_value,
                                             data=multipole_ell_emm)
...
class modes_array:
    ...
    def create_modes_array(self, ell_max=None, data_len=None):
        ...
        self.modes_data = np.zeros(
            (ell_max + 1, 2 * (ell_max + 1) + 1, data_len),
            dtype=np.complex128)
    ...
    def load_modes(
        ...
        if ftype == "generic":
            dataIO.load_gen_data_from_disk(self, label, data_dir, file_name,
                                           r_ext, ell_max, pre_key, modes_list,
                                           crop, centre, key_ex, r_ext_factor)
            ...
            for emm in emm_list:
                ...
                sp_data += np.multiply.outer(
                    Yslm_vec(spin_weight,
                             ...
                             emm=emm,
                             theta_grid=theta,
                             phi_grid=phi), self.mode(ell, emm))
                # message(sp_data)
    ...
    def to_frequency_basis(self):
        ...
        waveform_tilde_modes.create_modes_array(ell_max=self.ell_max,
                                                data_len=self.data_len)
        ...
        for mode in self.modes_list:
            ...
            for emm_value in emm_list:
                ...
                waveform_tilde_modes.set_mode_data(ell_value, emm_value,
                                                   freq_data)
    ...
    def to_time_basis(self):
        ...
        waveform_modes.create_modes_array(ell_max=self.ell_max,
                                          data_len=self.data_len)
    ...
    def extrap_to_inf(self,
        ...
        if method == "FO":
            ...
            extrap_method = partial(waveextract_to_inf_perturbative_one_order,
        ...
        extrap_wf = modes_array(label=f"{self.label} -> rPsi4_inf",
                                modes_list=self.modes_list,
                                ell_max=self.ell_max,
                                r_ext=self.r_ext)

        extrap_wf.create_modes_array(ell_max=self.ell_max,
                                     data_len=self.data_len)
    ...
    def supertranslate(self, supertransl_alpha_modes, grid_info, order=4):
        ...
        supertransl_factor = np.sum(
            np.array([
                np.power((-1j * omega_axis_2d * supertransl_alpha_sphere),
                         index) for index in range(order)
            ]),
            axis=0,
        )
    ...
    def get_strain_from_psi4(self, omega0="auto"):
        ...
        strain_waveform = modes_array(label="{} strain from Psi4".format(
            self.label),
                                      r_ext=self.r_ext,
                                      ell_max=8,
                                      modes_list=self.modes_list)
        ...
        strain_waveform.create_modes_array(ell_max=self.ell_max,
                                           data_len=data_len)
    ...
    def get_news_from_psi4(self, omega0="auto"):
        ...
        news_waveform = modes_array(label="{} news from Psi4".format(
            self.label),
                                    r_ext=500,
                                    ell_max=8,
                                    modes_list=self.modes_list)
        ...
        news_waveform.create_modes_array(ell_max=self.ell_max,
                                         data_len=data_len)
    ...
    def taper(self, zeros="auto"):
        ...
        for item in self.modes_list[:]:
            ...
            for emm in emm_list:
                ...
                if tapered_modes is None:
                    ...
                    tapered_modes.create_modes_array(ell_max=self.ell_max,
        ...
        # Set the time axis
        new_time_axis = np.arange(0, new_data_len * self.delta_t(),
                                  self.delta_t())
    ...
    def taper_tanh(self,
        ...
        for item in self.modes_list[:]:
            ...
            for emm in emm_list:
                ...
                _, tapered_data_re = taper_tanh(input_data_re,
                                                delta_t=self.delta_t(),
                                                duration=duration,
                                                sides=sides)
                _, tapered_data_im = taper_tanh(input_data_im,
                                                delta_t=self.delta_t(),
                                                duration=duration,
                                                sides=sides)
        ...
        for item in self.modes_list:
            ...
            for emm in emm_list:
                if filtered_modes is None:
                    ...
                    filtered_modes.create_modes_array(ell_max=self.ell_max,
                                                      data_len=self.data_len)

                # Get filtered mode data.
                filtered_data = low_cut_filter(self.mode(ell, emm),
                                               self.frequency_axis,
                                               omega0=omega0,
                                               order=order)
    ...
    def to_td_waveform(self,
        ...
        for ell, emm_list in self.modes_list:
            for emm in emm_list:
                ...
                # Y = Yslm(self.spin_weight, ell, emm)

                # all_yslm.append(Y)

                # wts_ele=0
                # for arr_ind in range(self.data_len):
                # message(Y)
                # message(Alm)
                # wts_ele +=  Alm * Y
                # wts.append(wts_ele)
        ...
        # 	accu_wts.append(wts[index].evalf(prec, subs={th: sp.Float(f"{theta}"), ph: sp.Float(f"{phi}")}))

        wts = np.complex256(wts)
        ...
        # plt.plot(taxis, wts.real, label=r"$h+$")
        # plt.plot(taxis, wts.imag, label=r"$h_\times$")
        # plt.legend()
        # plt.show()
