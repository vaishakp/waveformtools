...
class SimulationExplorer:
    ...
    def check_for_ecc_dir(self, path_of_dir_to_check):
        ...
        if not os.path.isdir(path_of_dir_to_check):
            flag = False
            message(f"{path_of_dir_to_check} is not a directory", message_verbosity=3)
        ...
        if 'Ecc0' not in subsubdirs:
            ...
            flag = False
            message(f"{path_of_dir_to_check} does not contain Ecc0", message_verbosity=3)
            return flag
        ...
        if 'DoMultipleRuns.input' not in ecc0_dir:
            ...
            flag = False
            message(f"{path_of_dir_to_check} does not contain DoMultipleRuns.input", message_verbosity=3)
        
        if flag:
            message(f"\t {path_of_dir_to_check} is a simulation directory", message_verbosity=3)
    ...
    def find_highest_ecc(self, sim_path):
        ...
        available_ecc_nums = [int(item) for item in available_ecc_nums if item.isdigit()]
    ...
    def discover_levels(self):
        ...
        for sim_name, sim_path in self.found_sim_paths.items():
            ...
            sim_levs = []
            path_to_highest_ecc_dir = sim_path.joinpath(f'Ecc{self.highest_ecc_nums[sim_name]}/Ev')
    ...
    def get_all_segments(self, ecc_dir):
        ...
        for lev in available_levs:
            ...
            this_lev_dirs = [item for item in available_lev_dirs if f'Lev{lev}' in item] 
    ...
    def parse_sim_params_input_file(self, path_to_params_input_file):
        ...
        message(f"Parsing simulation parameters.input file for {path_to_params_input_file}", message_verbosity=2)
    ...
    def parse_sim_target_params_input_file(self, path_to_target_params_input_file):
    ...
    def fetch_sim_params(self):
        ...
        for sim_name, sim_path in self.found_sim_paths.items():
            ...
            path_to_params_input_file = sim_path.joinpath(f'Ecc{ecc_num}').joinpath('Params.input')
            path_to_target_params_input_file = sim_path.joinpath(f'Ecc{ecc_num}').joinpath('TargetParams.input')
            
            one_sim_params_dict = self.parse_sim_params_input_file(path_to_params_input_file)
            
            one_sim_target_params_dict = self.parse_sim_target_params_input_file(path_to_target_params_input_file)
    ...
    def discover_extrapolated_sims(self):
        ...
        all_wfs = [item for item in all_wfs if os.path.isdir(waveforms_dir.joinpath(item))]
    ...
    def prepare_list_of_sims(self):
        ...
        for sim_name in self.found_sim_names:
            ...
            if (sim_name not in self.prepared_waveforms) and (self.available_sim_levs[sim_name] is not None):
    ...
    def prepare_waveforms(self, simulations_to_prepare=None):
        ...
        for sim_name in simulations_to_prepare:
            ...
            for lev in available_sim_levs:
                ...
                try:
                    wfp = PrepareSXSWaveform(sim_name=sim_name, sim_dir=self.found_sim_paths[sim_name], lev=lev, out_dir=self.prepared_waveforms_dir)
                    ...
                    message(excep)
                    message(f"Waveform processing failed sim {sim_name}, lev {lev}")
