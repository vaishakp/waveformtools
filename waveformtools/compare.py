...
def plot_mode_differences(
	waveforms, nmodes=3, save_fig=False, xlabel="t/M", ylabel=r"\Psi_{4}^{(\ell m)}", labels=None, xlim=[-1000, 100]
):
	...
	for item in modes_to_plot[:nmodes]:
		...
		for emm in emm_list:
			...
			for index, wfx in enumerate(waveforms[1:]):
				...
				mode_valuesx = wfx.r_ext * wfx.mode(ell, emm)
				...
				start_ind = int((-new_time_axis[0] - 1000) / new_delta_t)

				# print(start_ind)
				# Remove the man phase shift
				mean_phase_shift = np.mean(
					mode_phase_resamx[start_ind : start_ind + 10] - mode_phase_resam0[start_ind : start_ind + 10]
				)
	...
	ax1.set_ylabel(rf"$\vert{ylabel}\vert $")
	ax2.set_xlabel(xlabel)
	ax2.set_ylabel(rf"$\Phi({ylabel})$")
