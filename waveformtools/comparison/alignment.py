...
Full time-shift optimization and SO(3) rotations are intentionally left for
later optimizer/rotation PRs.
"""
...
def prepare_mode_data(
    ...
    dt_a, uniform_a = sampling_interval(time_a_original, alignment.time_axis_rtol, alignment.time_axis_atol)
    dt_b, uniform_b = sampling_interval(time_b_original, alignment.time_axis_rtol, alignment.time_axis_atol)

    if alignment.time_domain_policy == "error":
        reference_grid = _strict_common_grid(time_a, time_b, alignment)
        arrays_a = {mode: np.asarray(modes_a.mode(*mode), dtype=np.complex128) for mode in selected_modes}
        arrays_b = {mode: np.asarray(modes_b.mode(*mode), dtype=np.complex128) for mode in selected_modes}
    ...
    diagnostics = TimeAxisDiagnostics(
        ...
        uniform_a=uniform_a,
        uniform_b=uniform_b,
        original_range_a=(float(time_a_original[0]), float(time_a_original[-1])),
        original_range_b=(float(time_b_original[0]), float(time_b_original[-1])),
...
def reference_time(modes_obj: Any, selected_modes: Sequence[tuple[int, int]], mode: str) -> float:
...
def overlap_interval(time_a: np.ndarray, time_b: np.ndarray) -> tuple[float, float] | None:
    ...
    *,
    ...
    if method == "cubic":
        try:
            ...
            raise ImportError("cubic resampling requires scipy.interpolate.CubicSpline") from exc
...
def _strict_common_grid(time_a: np.ndarray, time_b: np.ndarray, alignment: AlignmentSpec) -> np.ndarray:
    ...
    if not np.allclose(time_a, time_b, rtol=alignment.time_axis_rtol, atol=alignment.time_axis_atol):
        raise ValueError(
...
def _require_compatible_sampling(dt_a: float | None, dt_b: float | None, alignment: AlignmentSpec) -> None:
    if dt_a is None or dt_b is None:
        raise ValueError("Cannot check sampling-rate compatibility with fewer than two samples.")
    if not np.isclose(dt_a, dt_b, rtol=alignment.time_axis_rtol, atol=alignment.time_axis_atol):
        raise ValueError(
