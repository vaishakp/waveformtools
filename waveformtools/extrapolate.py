...
def waveextract_to_inf_perturbative_twop5_order(rPsi4_rlm, delta_t, areal_radius=500, Mass=1, spin=0, ell=2, emm=2):
	''' Extract a numerical waveform to null infinity using perturbative techniques. This is :
		* accurate to second order in 1/r.
	...
	timeaxis			=	np.linspace(0, len(rPsi4_rlm)*delta_t, )
	# Assigning the terms. Each set of subterms in a pair of paranthesis is a term.
	term_1_prefac		=	(1 - 2*Mass/areal_radius)
	suberm_1_1			=	rPsi4_rlm
	subterm_1_2_prefac	=	(ell -1)* (ell +2)/ (2*areal_radius)
	subterm_1_2			=	waveformtools.integrate.fixed_frequency_integrator(rPsi4_rlm, delta_t, omega0=0.015) #Integral_rPsi4_rlm
	subterm_1_3_prefac	=	(ell-1) * (ell+2) * (ell**2 + ell -4)/ (8 * areal_radius**2)
	subterm_1_3			=	waveformtools.integrate.fixed_frequency_integrator(rPsi4_rlm, delta_t, order=2, omega0=0.015) #Double_integral_rPsi4_rlm

	term_1				=	term_1_prefec* (subterm_1_1 + subterm_1_2_prefac * subterm_1_2 + subterm_1_3_prefac*subterm_1_3)


	term_2_prefac		=	(2 * 1j * spin/ (ell + 1)**2) * np.sqrt(((ell+3) * (ell - 1) * (ell + emm + 1) * (ell - emm + 1)/ ((2* ell + 1) * (2*ell + 3)))
	subterm_2_1			=	waveformtools.differentiate.differentiate_cwaveform(timeaxis, rPsi4_rlm) # Differential of waveform
	...
	term_3_prefac		=	(-2*1j*spin/ell**2)*np.sqrt(((ell + 2) * (ell - 2) * (ell + m) * (ell - m))/ ((2*ell - 1)* (2*ell + 1)))
	subterm_3_1			=	subterm_2_1
	subterm_3_2_prefac	=	(-(ell-2) * (ell +1)/ areal_radius)
	subterm_3_2			=	subterm_1_1

	term_3				=	term_3_prefac * (subterm_3_1 + subterm3_2_prefac * subterm_3_2)
...
def waveextract_to_inf_perturbative_two_order(rPsi4_rlm, delta_t, areal_radius=500, Mass=1, spin=0, ell=2, emm=2):
	...
	subterm_2_1			=	waveformtools.integrate.fixed_frequency_integrator(rPsi4_rlm, delta_t, omega0=0.015) #Integral_rPsi4_rlm
	term_2				=	term_2_prefac * subterm_2_1

	term_3_prefac		=	(ell-1) * (ell + 2) * (ell**2 + ell - 4)/(8 * areal_radius**2)
	subterm_3_1			=	waveformtools.integrate.fixed_frequency_integrator(rPsi4_rlm, delta_t, order=2, omega0=0.015) #Double_Integral_rPsi4_rlm
...
def waveextract_to_inf_perturbative_one_order(u_ret, rPsi4_rlm, areal_radius=500, Mass=1, ell=2, emm=2):
	...
	A_lm, Phi_lm			=	waveformtools.xtract_camp_phase

	# Get the time stepping
	delta_t					=	u_ret[1]-u_ret[0]

	# Get the waveform instantaneous frequency
	omega_lm				=	waveformtools.get_waveform_angular_frequency(rPsi_rlm, delta_t=delta_t, timeaxis=u_ret)
