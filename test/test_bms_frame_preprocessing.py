...
def make_diagnostics(vector) -> BMSFrameDiagnosticsResult:
    return BMSFrameDiagnosticsResult(
        energy_radiated=0.1,
        radiated_linear_momentum=np.asarray(vector, dtype=float),
        kick_velocity=np.asarray(vector, dtype=float) / 0.95,
        angular_momentum_radiated=np.asarray(vector, dtype=float),
